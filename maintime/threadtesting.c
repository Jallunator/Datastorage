...
/*   Updated: 2023/09/05 18:47:19 by Alex P           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *timeThread(void *arg) {
    while (!shouldExit) {
        ...
        mvwprintw(timewin, 32, 10, "Time:                    "); // Clear previous time
        wattroff(timewin, COLOR_PAIR(1));

        // Print the updated time in the window
        wattron(timewin, COLOR_PAIR(1));
        mvwprintw(timewin, 32, 10, "Time: %s", timeString);
        wattroff(timewin, COLOR_PAIR(1));
...
int timeswindow(void) 
{
    ...
    WINDOW *timewin = createtimewin(&yMax, &xMax);
     ...
     // Create a separate thread for the time display
    pthread_t thread;
    ...
    while (1) {
        ...
        if (choice == 'Q' || choice == 'q') {
            shouldExit = 1; // Set the exit flag to terminate the timeThread
            break; // Exit the loop on 'Q' or 'q'
        }
    }
    
    pthread_join(thread, NULL); // Wait for the timeThread to finish
    endwin();
    return choice;
}
